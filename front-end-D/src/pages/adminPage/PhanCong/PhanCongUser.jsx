...
export default function PhanCongUser() {
    ...
    const fetchData = async () => {
        try {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                promises.push(fetch(`/api/admin/phancong/${id}/${formatday(ngayphancong)}/${formatday(startDate)}`, {
                    method: 'GET',
                    headers: {
    ...
    return (
        <div className="bg-white border shadow-sm m-2 z mt-24 p-2 md:p-10 rounded-3xl">
            <button className='border rounded-lg border-slate-400 text-white text-base font-bold px-5 py-2 my-3 bg-green-500 hover:bg-green-700'
